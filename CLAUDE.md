...
- **API Integration**: Gemini API (requires GEMINI_API_KEY in .env.local)

### Component Structure
The application follows a modular component architecture with each section as a separate component:

- `App.tsx` - Main application component that orchestrates all sections
- `components/` - Landing page sections:
  - `Header.tsx` - Fixed header with logo and CTA
  - `Hero.tsx` - First view with main value proposition
  - `Empathy.tsx` - Pain points section
  - `Features.tsx` - Service features (USP)
  - `Benefits.tsx` - Implementation benefits
  - `Conditions.tsx` - Contract terms
  - `Process.tsx` - Implementation flow
  - `Trust.tsx` - Trust elements (testimonials, portfolio)
  - `Closing.tsx` - Final CTA with form
...
Target platform: Vercel (as specified in project requirements)

## Important Notes

- Components use Tailwind utility classes for styling
- No external CSS files; all styling is inline
- Form submissions in `Closing.tsx` component handle contact inquiries
- Privacy policy consent checkbox required before form submission
